...
for (let i = 0; i < 50; i++) {
  // make the graves angle randomly it goes in side circle so math.pi
  const angle = Math.random() * Math.PI * 2;
  // set positions randomly inside the circle it goes between 3 and 9 the default circle is 1
  const radius = 3 + Math.random() * 6;
  const x = Math.sin(angle) * radius;
  const z = Math.cos(angle) * radius;
  const grave = new THREE.Mesh(graveGeometry, graveMaterial);
  // set the position and add the graves
...
// Oprimize shadows for more performance
// ...
